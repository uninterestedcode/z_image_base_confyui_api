...
CMD ["sh", "-c", "echo 'Starting ComfyUI...' && python /comfyui/main.py --listen 0.0.0.0 --port 8188 > /tmp/comfyui.log 2>&1 & COMFYUI_PID=$! && echo 'Waiting for ComfyUI to be ready...' && for i in $(seq 1 60); do if curl -s http://127.0.0.1:8188/system_stats > /dev/null 2>&1; then echo 'ComfyUI is ready!'; break; fi; echo 'Waiting for ComfyUI... ($i/60)'; sleep 1; done && if ! curl -s http://127.0.0.1:8188/system_stats > /dev/null 2>&1; then echo 'ERROR: ComfyUI failed to start'; cat /tmp/comfyui.log; exit 1; fi && echo 'Starting RunPod serverless handler...' && python handler.py"]
